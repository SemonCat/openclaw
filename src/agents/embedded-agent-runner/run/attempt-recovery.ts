...
> {
  ...
  // A long Codex turn can hit a subscription boundary only after it has already
  // persisted side-effecting tool results. Replaying the original prompt would
  // duplicate those tools, but rotating the profile and continuing from the
  // settled transcript is safe. Keep every other replay-unsafe prompt failure
  ...
  if (
    ...
    const promptFailureOutcome = await handleEmbeddedPromptFailure({
      ...
      // The outer fallback normally retries the original prompt. Arm its
      // continuation-only path before crossing models so settled tools are not
      // repeated after every authorized profile is exhausted.
      if (
        settledRateLimitPromptFailure &&
